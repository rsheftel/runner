...
import montauk.market.datetime as mdatetime
...
class MetricRunner(RunnerBase):
    ...
    def add_metrics(self, container, metric, arguments, **kwargs):
        """
        Add metrics via the MetricContainer class. This method will instantiate a new MetricContainer object, the
        container class, and populate that container with the Metric class, metric, and each of the arguments. The
        optional kwargs will be passed to the MetricContainer which will use them to initialize the Metrics.

        :param container: MetricContainer class
        ...
        if not self.market_data_manager:
            raise RuntimeError('Cannot add metrics before setting up the market data!')

        log.info(f'Initializing MetricContainer: {container}')
        self._containers.append(container(self.market_data_manager, metric, arguments, **kwargs))
    ...
    def run(self, bartimes):
        self._event_looper = tw.MetricProcessor(self._containers, self.market_data_manager)

        log.info('Starting MetricContainers')
        self._event_looper.start()

        log.info('beginning run from {} to {} at frequency {}'.format(bartimes[0], bartimes[-1], self.min_frequency()))
        for bartime in bartimes:
            log.info(f'running bar: {bartime}')
            self._market_data_manager.bartime = bartime
            self._event_looper.process_bar(self.min_frequency())

        log.info('Stopping MetricContainers')
        self._event_looper.stop()
